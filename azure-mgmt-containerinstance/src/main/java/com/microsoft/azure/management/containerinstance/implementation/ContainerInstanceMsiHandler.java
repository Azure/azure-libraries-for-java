...
class ContainerInstanceMsiHandler extends RoleAssignmentHelper {
    ...
    ContainerInstanceMsiHandler(final GraphRbacManager rbacManager, ContainerGroupImpl containerGroup) {
        ...
        this.userAssignedIdentities = new HashMap<String, ContainerGroupIdentityUserAssignedIdentitiesValue>();
    }
     ...
     * port as 50342.
     *
     * @return ContainerInstanceMsiHandler
     */
    ContainerInstanceMsiHandler withLocalManagedServiceIdentity() {
        this.initContainerInstanceIdentity(ResourceIdentityType.SYSTEM_ASSIGNED);
        return this;
    }

    ContainerInstanceMsiHandler withoutLocalManagedServiceIdentity() {
        if (this.containerGroup.inner().identity() == null
                ...
                || this.containerGroup.inner().identity().type() == ResourceIdentityType.USER_ASSIGNED) {
            return this;
        } else if (this.containerGroup.inner().identity().type() == ResourceIdentityType.SYSTEM_ASSIGNED) {
    ...
    ContainerInstanceMsiHandler withNewExternalManagedServiceIdentity(Creatable<Identity> creatableIdentity) {
    ...
    ContainerInstanceMsiHandler withExistingExternalManagedServiceIdentity(Identity identity) {
        this.initContainerInstanceIdentity(ResourceIdentityType.USER_ASSIGNED);
